...
void BinaryTree<T>::add(TreeNode<T>*& node, T& data)  //метод добавления элемента в дерево
{
    if (!node)
        ...
        data.getAccountData()->getId() < node->data.getAccountData()->getId() ? add(node->left, data) : add(node->right, data);
}
...
template<typename T>
T& BinaryTree<T>::Iterator::operator++(int) //оператор постинкремента, чтобы сдвинуться вперед по дереву
{
...
template<typename T>
T& BinaryTree<T>::Iterator::operator+(int value) //оператор +, чтобы сдвинуться вперед по дереву с шагом
{
    for (Iterator it = _current; it != it + value; it++)
        _current = _current->right;
    return _current->data;
}
