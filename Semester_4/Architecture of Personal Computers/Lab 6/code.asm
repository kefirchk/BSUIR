...
START:
    ...
    mov ds,ax                                   
    mov es,ax                          
    lea dx,MSG_ENTER                            ;сообщение о времени нахождения
    mov ah,9h
    int 21h

    call INPUT                                 ;ввод данных с клавиатуры       
    mov ds:[TIME], al
    call FILL_CR_0_BUFFER_RM                   ;чтение cr0
    lea dx, MSG_BUFFER_CR_0_RM                 ;сообщение о выводе cr0
    mov ah, 9h
    int 21h
    lea dx, BUFFER_CR_0_RM                     ;вывод cr0
    mov ah, 9h
    int 21h
    lea dx, MSG_HELLO
    mov ah,9h
    int 21h
    mov ah,7h                                  ;press any key...
    int 21h          
...
ENABLE_A20:                                     ;Открыть линию A20 - она отвечает за доступ к памяти, физические адреса которой превышают 1 Мбайт
    in  al,92h                                                                              
...
LOAD_GDT:                                       ;Загружаем глобальную таблицу дескрипторов            
    mov ax,DATA
    mov dl,ah
    xor dh,dh
    shl ax,4
...
WRITE_GDT:                                      ;вычислим и загрузим в GDT линейный адрес сегмента глобальных таблиц
    lea bx,GDT_GDT                              ;bx -  смещение дескриптора
    mov ax,si
    mov dx,di
    add ax,offset GDT
    adc dx,0
    mov [bx][S_DESC.BASE_L],ax                  ;загрузка младшей части базы
    mov [bx][S_DESC.BASE_M],dl                  ;загрузка средней части базы
    mov [bx][S_DESC.BASE_H],dh                  ;загрузка старшей части базы

WRITE_CODE_RM:                                  ;Заполнить дескриптор сегмента кода реального режима
    lea bx,GDT_CODE_RM                          ;bx - смещение дескриптора
    mov ax,cs
    xor dh,dh
    mov dl,ah
    shl ax,4
...
WRITE_DATA:                                     ;Записать дескриптор сегмента данных
    lea bx,GDT_DATA                             ;bx - смещение дескриптора
    mov ax,si
    mov dx,di
    mov [bx][S_DESC.BASE_L],ax                  ;загрузка младшей части базы
    mov [bx][S_DESC.BASE_M],dl                  ;загрузка средней части базы
...
WRITE_STACK:                                    ;Записать дескриптор сегмента стека
    lea bx, GDT_STACK                           ;bx - смещение дескриптора
    mov ax,ss
    xor dh,dh
    mov dl,ah
    shl ax,4
...
WRITE_CODE_PM:                                  ;Записать дескриптор кода защищенного режима
    lea bx,GDT_CODE_PM                          ;bx - смещение дескриптора
    mov ax,CODE_PM       
    xor dh,dh
    mov dl,ah
    shl ax,4
...
WRITE_IDT:                                      ;Записать дескриптор IDT
    lea bx,GDT_IDT                              ;bx - смещение дескриптора
    mov ax,si
    mov dx,di
    add ax,OFFSET IDT
    adc dx,0
    ...
    mov [bx][S_DESC.BASE_M],dl                  ;загрузка средней части базы
    mov [bx][S_DESC.BASE_H],dh                  ;загрузка старшей части базы
    mov IDTR.IDT_L,ax                           ;загрузим вудущий
    mov IDTR.IDT_H,dx                           ;загрузим ведомый
...
FILL_IDT:                                       ;Заполнить таблицу дескрипторов векторов прерываний
    ...
    irpc    N, 234567                           ;Заполнить вектора IRQ2 - IRQ7 заглушками
    ...
    irpc    N, 89ABCDEF                         ;Заполнить вектора IRQ8-IRQF заглушками
...
;Подготовка системных регистров и переход в РМ:

    lgdt fword ptr GDT_GDT                      ;Загрузить регистр GDTR
    lidt fword ptr IDTR                         ;Загрузить регистр IDTR, fword == far word
...
BACK_TO_RM:                                     ;Точка возврата в реальный режим
	...
	or	AL,10000000b                            ;Установить 7 бит в 1 для запрета немаскируемых прерываний
	out	70h,AL                 
	nop                                         ;Ничего не делать(для задержки)
...
;-------------------------------------------------------------------
FILL_CR_0_BUFFER_RM proc near                   ;заполнение cr0
	push eax
	push esi
	push dx
...
ENTER_PM:                                       ;Точка входа в защищенный режим
    ...
    call FILL_CR_0_BUFFER                       ;обновить cr0
    mov edi, 640                                ;курсор на новую строку
    lea esi, MSG_BUFFER_CR_0_PM
    call BUFFER_OUTPUT
    mov edi, 672
    lea esi, BUFFER_CR_0
    call BUFFER_OUTPUT
...
DATA_BEGIN      = $
    ...
    MSG_BUFFER_CR_0_RM  db "CR0 [b0...b31]: $"
    MSG_BUFFER_CR_0_PM  db "CR0 [b0...b31]: ",0
