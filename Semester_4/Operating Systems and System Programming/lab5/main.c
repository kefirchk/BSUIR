...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        if (!strcmp(option, "h\n")) {                 // h
            ...
            change_buffer_size(&queue, queue.size + INCREASE_NUMBER);
            	
            pthread_mutex_lock(&stdout_mutex);
            printf("Buffer is increased, buffer size: %d\n", queue.size);
            pthread_mutex_unlock(&stdout_mutex);
            ...
            if(queue.size > MIN_BUFFER_SIZE) {
            	pthread_mutex_lock(&write_mutex);
            	pthread_mutex_lock(&read_mutex);
            	
            	change_buffer_size(&queue, queue.size - DECREASE_NUMBER);
            	
            	pthread_mutex_lock(&stdout_mutex);
            	printf("Buffer is reduced, buffer size: %d\n", queue.size);
            	pthread_mutex_unlock(&stdout_mutex);
                ...
                pthread_mutex_lock(&stdout_mutex);
            	printf("Error: buffer cannot be reduced\n");
            	pthread_mutex_unlock(&stdout_mutex);
            }
