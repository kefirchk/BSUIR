...
def main():
    ...
    while True:
        ...
        for i in range(len(threads) - MIN_T):
            if summary > MIN_T and not globalIsWork[len(threads) - i - 1]:
                globalToKill[len(threads) - i - 1] = True
                threads[len(threads) - i - 1].join()
                threads_to_remove.append(len(threads) - i - 1)
