...
void Frame::decodeHammingCode()
{
    ...
    //std::reverse(unmatchedControlBits.begin(), unmatchedControlBits.end());
    //bool ok;
    //short fcsValue = unmatchedControlBits.toInt(&ok, 2);
    //for(int i = 0; i < 4; ++i)
    //{
    //    if(unmatchedControlBits[i] == '1')
    //        fcsValue -= i + 1;
    //}
    //std::reverse(unmatchedControlBits.begin(), unmatchedControlBits.end());
    //this->setFcs(QString::number(fcsValue, 2).rightJustified(4, '0'));//newFrame.getFcs());
    //this->setFcs(newFrame.getFcs());
...
QString Frame::sendFrame(QSerialPort& port)
{
