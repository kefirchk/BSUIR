...
void Notebook::setTypeOfNotebook(const char* type)             //метод задания типа ноутбука
{
	strcpy_s(typeOfNotebook, strlen(type) + 1, type);
}
...
ifstream& operator>>(ifstream& fin, Notebook& notebook)        //оператор >> для ввода
{
	...
	char* s = notebook.typeOfNotebook;
	delete_spaces(s);
	skip_spaces(fin);
...
void Notebook::cleanSomeFields(Notebook& notebook)
{
	if (!strcmp(notebook.brand, ""))
	{
		strcpy_s(this->brand, strlen(this->brand), notebook.brand);
	}
	if (notebook.weight == 0)
	{
		this->weight = 0;
	}
	if (notebook.batteryCapacity == 0)
	{
		this->batteryCapacity = 0;
	}
	if (notebook.keyboardBacklight == '?')
	{
		this->keyboardBacklight = '?';
	}
	if (!strcmp(notebook.typeOfNotebook, ""))
	{
		strcpy_s(this->typeOfNotebook, strlen(this->typeOfNotebook), "");
	}
}
...
bool Notebook::operator<(Notebook& notebook)
{
	if (strcmp(notebook.brand, ""))
	{
		if (strcmp(this->brand, notebook.brand) < 0)
			return true;
		return false;
	}
	if (notebook.weight != 0)
	{
		if (this->weight < notebook.weight)
			return true;
		return false;
	}
	if (notebook.batteryCapacity != 0)
	{
		if (this->batteryCapacity < notebook.batteryCapacity)
			return true;
		return false;
	}
	if (notebook.keyboardBacklight != '?')
	{
		if (this->keyboardBacklight < notebook.keyboardBacklight)
			return true;
		return false;
	}
	if (strcmp(notebook.typeOfNotebook, "") != 0)
	{
		if (strcmp(this->typeOfNotebook, notebook.typeOfNotebook) < 0)
			return true;
		return false;
	}
	return false;
...
void Notebook::set_sort_parametres()   //метод выбора параметров поиска
{
	Interface<int> text;
	int choice;          //номер параметра
	rewind(stdin);       //очистка буфера
	...
	cout << "5 - тип ноутбука" << endl;
	text.setColor(7);    //белый
	choice = get_int(1, 5); //ввод номера параметра поиска
	switch (choice)
	{
	case 1: this->setBrand("-"); break;
	case 2: this->setWeight(1); break;
	case 3: this->setBatteryCapacity(1); break;
	case 4: this->setKeyboardBacklight('-'); break;
	case 5: this->setTypeOfNotebook("simple"); break;
	default: break;
	}
}